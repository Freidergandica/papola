...
import { MapPin, Star, Power, PowerOff, Edit2, Tag, Hash } from 'lucide-react'
...
export default async function StoresPage() {
  ...
  // Fetch all stores with admin client
  ...
  async function toggleStoreStatus(storeId: string, currentStatus: boolean) {
    'use server'
    const admin = createAdminClient()
    await admin.from('stores').update({ is_active: !currentStatus }).eq('id', storeId)
    ...
    <div>
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-500 mt-2">Activa, desactiva o edita los negocios de la plataforma.</p>
        </div>
        <Link
          ...
          <div key={store.id} className={`bg-white rounded-2xl shadow-sm border ${store.is_active ? 'border-gray-100' : 'border-red-100 bg-red-50/30'} overflow-hidden flex flex-col`}>
            <div className="h-40 bg-gray-200 relative">
               ...
               {store.affiliate_number && (
                 <div className="absolute top-4 left-4 bg-papola-blue/90 backdrop-blur-sm px-2.5 py-1 rounded-full flex items-center shadow-sm">
                    <Hash className="w-3 h-3 text-white mr-0.5" />
                    <span className="text-[10px] font-bold text-white">{store.affiliate_number}</span>
                 </div>
               )}
            </div>
            ...
            <div className="p-6 flex-1 flex flex-col">
              ...
              <div className="mt-auto flex gap-2 pt-4 border-t border-gray-100">
                 ...
                 <form action={toggleStoreStatus.bind(null, store.id, store.is_active)}>
                   <button
                     type="submit"
                     className={`flex items-center justify-center py-2 px-4 rounded-lg font-medium text-sm transition-colors ${store.is_active ? 'bg-red-50 hover:bg-red-100 text-red-600' : 'bg-papola-green-20 hover:bg-papola-green-40 text-papola-green'}`}
                     title={store.is_active ? 'Desactivar' : 'Activar'}
                   >
                      {store.is_active ? <PowerOff className="w-4 h-4" /> : <Power className="w-4 h-4" />}
                   </button>
                 </form>
